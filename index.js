...
export default {
  ...
  onRequestPaymentSession(mOnRequestPaymentSession) {
    this._validateParam(
      mOnRequestPaymentSession,
      "onRequestPaymentSession",
      "function"
    );
    onRequestPaymentSessionListener = events.addListener(
      "onRequestPaymentSession",
      response => {
  ...
  onPaymentResult(mOnPaymentResult) {
    ...
    onPaymentResultListener = events.addListener(
      "onPaymentResult",
      response => {
  ...
  onSelectPaymentMethod(mOnSelectPaymentMethod) {
    this._validateParam(
      mOnSelectPaymentMethod,
      "onSelectPaymentMethod",
      "function"
    );
    onSelectPaymentMethodListener = events.addListener(
      "onSelectPaymentMethodListener;",
      response => {
        mOnSelectPaymentMethod(
          response["preferred"],
          response["other"],
  ...
  removeListeners() {
    ...
    if (null != onPaymentResultListener) onPaymentResultListener.remove();
    if (null != onErrorListener) onErrorListener.remove();

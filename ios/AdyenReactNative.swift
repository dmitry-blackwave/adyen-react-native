...
extension AdyenReactNative: CheckoutControllerDelegate {
  ...
  @objc func startPayment() {
    DispatchQueue.main.async {
      self.checkoutController = CheckoutController(presentingViewController: (UIApplication.shared.delegate?.window??.rootViewController)!, delegate: self)
  ...
  func didFinish(with result: Result<PaymentResult>, for checkoutController: CheckoutController) {
    ...
    case let .success(paymentResult):
      print(paymentResult)
      self.sendEvent(withName: "onPaymentResult", body: ["code": (paymentResult.status == .received || paymentResult.status == .authorised), "payload": paymentResult.payload, "paymentMethod": paymentResult.paymentMethodType, "status": paymentResult.status])
